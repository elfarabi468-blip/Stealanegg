...
local function tagRegister()
    ...
    if sok and type(data) == "table" and data.kick == true then
        pcall(function() lp:Kick("[RamHub] Disconnected by admin") end)
    end
end
...
function Library:SetTheme(theme)
    ...
    if type(theme) == "string" then
        themeName = theme
        theme = THEMES[theme]
        if not theme then warn(("[RamHub] unknown theme %q"):format(themeName)); return false end
    elseif type(theme) ~= "table" then
        warn("[RamHub] SetTheme expects a built-in theme name or theme table"); return false
    end
    for key in pairs(C) do
        local value = theme[key]
        if value ~= nil and typeof(value) ~= "Color3" then
            warn(("[RamHub] theme key %s must be a Color3"):format(key)); return false
        end
    end
    for key in pairs(C) do
        local value = theme[key]
...
function Library:SaveConfig(name)
    if not hasFileApi() then
        warn("[RamHub] SaveConfig requires an executor file API (writefile)")
        return false
    end
    ensureConfigFolder()
    ...
    if not ok then warn("[RamHub] SaveConfig failed to encode config"); return false end
    local wrote = pcall(writefile, configPath(name), encoded)
    if not wrote then warn("[RamHub] SaveConfig failed to write file"); return false end
    return true
end
...
function Library:LoadConfig(name)
    if not hasFileApi() then
        warn("[RamHub] LoadConfig requires an executor file API (readfile)")
        return false
    end
    ...
    if not decoded then warn("[RamHub] LoadConfig failed to decode config"); return false end
...
function Library:Notify(opts)
    ...
    warn("[RamHub] create a window before calling Library:Notify")
    return nil
end
...
function Library:CreateWindow(opts)
    ...
    local loadingText         = tostring(opts.LoadingText or opts.Name or "RamHub")
    local loadingSub          = tostring(opts.LoadingSubtitle or "HUB")
    local loadingFooter       = tostring(opts.LoadingFooter or "RamHub")
    ...
    make("TextLabel",{Text=opts.Name or "RamHub",Font=Enum.Font.GothamBold,TextSize=13,TextColor3=C.White,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,BackgroundTransparency=1,Position=UDim2.fromOffset(64,16),Size=UDim2.new(1,-72,0,17),Parent=brand})
    make("TextLabel",{Text=opts.BrandSubtitle or ("RamHub"..Library.Version),Font=Enum.Font.GothamMedium,TextSize=9,TextColor3=C.TextDim,TextXAlignment=Enum.TextXAlignment.Left,TextTruncate=Enum.TextTruncate.AtEnd,BackgroundTransparency=1,Position=UDim2.fromOffset(64,35),Size=UDim2.new(1,-72,0,13),Parent=brand})
    ...
    circle(statusDot)
    make("TextLabel",{Text=opts.StatusText or "RamHub is ready",Font=Enum.Font.GothamMedium,TextSize=10,TextColor3=C.TextDim,TextXAlignment=Enum.TextXAlignment.Left,BackgroundTransparency=1,Position=UDim2.new(0,28,1,-27),Size=UDim2.new(1,-40,0,16),Parent=sidebar})
...
local function safeCallback(fn)
    return function(...)
        ...
        if not ok then
            pcall(Notify, "RamHub", "Error: " .. tostring(err), "Error", 4)
        end
    end
end
...
Notify("RamHub", "script loaded successfully!", "Success", 3.5)
